...
namespace BSMemory {
	[[nodiscard]]
	extern __declspec(allocator) void* malloc(std::size_t size);
	[[nodiscard]]
	extern __declspec(allocator) void* calloc(std::size_t size);
	[[nodiscard]]
	extern __declspec(allocator) void* aligned_alloc(size_t alignment, std::size_t size);
	[[nodiscard]]
	extern __declspec(allocator) void* realloc(void* ptr, std::size_t new_size);

	extern void  free(void* ptr);

	extern std::size_t msize(void* ptr);

	template <typename T_Data>
	[[nodiscard]]
	__declspec(restrict) __declspec(allocator) T_Data* malloc() {
		return static_cast<T_Data*>(BSMemory::malloc(sizeof(T_Data)));
	};

	template <typename T_Data>
	[[nodiscard]]
	__declspec(restrict) __declspec(allocator) T_Data* malloc(std::size_t count) {
		return static_cast<T_Data*>(BSMemory::malloc(sizeof(T_Data) * count));
	};

	template <typename T, const uint32_t ConstructorPtr = 0, typename... Args>
	[[nodiscard]]
	__declspec(restrict)T* create(Args &&... args)
	{
...
_VCRT_EXPORT_STD void __CRTDECL operator delete[](void* _Block, size_t _Size) noexcept { BSMemory::free(_Block); }\
_VCRT_EXPORT_STD void __CRTDECL operator delete(void* _Block, ::std::align_val_t _Al) noexcept { BSMemory::free(_Block); } \
_VCRT_EXPORT_STD void __CRTDECL operator delete(void* _Block, ::std::align_val_t _Al, ::std::nothrow_t const&) noexcept { BSMemory::free(_Block); } \
_VCRT_EXPORT_STD void __CRTDECL operator delete[](void* _Block, ::std::align_val_t _Al) noexcept { BSMemory::free(_Block); } \
_VCRT_EXPORT_STD void __CRTDECL operator delete[](void* _Block, ::std::align_val_t _Al, ::std::nothrow_t const&) noexcept { BSMemory::free(_Block); } \
_VCRT_EXPORT_STD void __CRTDECL operator delete(void* _Block, size_t _Size, ::std::align_val_t _Al) noexcept { BSMemory::free(_Block); } \
_VCRT_EXPORT_STD void __CRTDECL operator delete[](void* _Block, size_t _Size, ::std::align_val_t _Al) noexcept { BSMemory::free(_Block); }